...
app.use(
  "/test1",
  (req, res, next) => {
    res.setHeader("Set-Cookie", ["sessionId=test-123; Path=/", "userPrefs=dark; Path=/; SameSite=None"]);
    next();
  },
  express.static(path.join(__dirname, "test1"))
);
...
app.use(
  ...
  (req, res, next) => {
    ...
    res.setHeader(
      ...
      "default-src 'self' https:; script-src 'self' https:; img-src 'self' https:;"
    );
