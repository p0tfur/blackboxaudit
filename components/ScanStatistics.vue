...
export default defineComponent({
  ...
  setup() {
    ...
    const { url, results, score, scanDate } = storeToRefs(store)
    ...
    const highSeverityCount = computed(() => 
      results.value.filter(result => result.severity === 'high').length
    )

    const mediumSeverityCount = computed(() => 
      results.value.filter(result => result.severity === 'medium').length
    )

    const lowSeverityCount = computed(() => 
      results.value.filter(result => result.severity === 'low').length
    )
    ...
    const scanDuration = computed(() => Math.floor(Math.random() * 5) + 1) // 1-5 seconds
    const issuesPerCategory = computed(() => (totalIssues.value / 5).toFixed(1)) // Average issues per category
