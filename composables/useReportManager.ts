...
export const useReportManager = () => {
  const exportToPDF = async (report: ScanReport) => {
    ...
    const scoreY = 60
    doc.setFontSize(20)
    const scoreColor = report.score >= 90 ? [39, 174, 96] as [number, number, number] : 
                      report.score >= 70 ? [241, 196, 15] as [number, number, number] : 
    ...
    doc.rect(margin + (report.score / 100) * contentWidth, scoreY + 5, 
             contentWidth - (report.score / 100) * contentWidth, 5, 'F')
